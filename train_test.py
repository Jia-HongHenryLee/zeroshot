...
if __name__ == '__main__':
    ...
    if LOAD_MODEL is None:
        model = visual_semantic_model(pretrained=True, k=CONFIG['k'], d=CONFIG['d'])
        model = model.to(DEVICE)

    else:
        print("Loading pretrained model")
        model = visual_semantic_model(pretrained=False, k=CONFIG['k'], d=CONFIG['d'])
    ...
    CONFIG['loss_args']['alpha'] = CONFIG['loss_args']['alpha'] * datasets['train'].pos_weight
    ...
    for epoch in range(CONFIG['start_epoch'], CONFIG['end_epoch']):
        ...
        for g in [False, True]:
            ...
            train_prf1 = utils.cal_prf1(train_metrics, general=g)
