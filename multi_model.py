...
def model_epoch(mode, epoch, loss_name, model, loss_args, data_loader, concepts, optimizer, writer):
    ...
    class HingeRankLoss():
        ...
        def __init__(self, concept, loss_args):
            ...
            self.alpha = torch.cuda.FloatTensor([loss_args['alpha'], 1])
    ...
    for batch_i, batch_data in enumerate(data_loader, 1):
        ...
        if mode == 'train':
            ...
            l1_params = torch.cat([x.view(-1) for x in model.transform.parameters()])
            l1_norm = torch.norm(l1_params, p=1) * 0.0000001
            loss += l1_norm
